...
const categoryChangeHandler = (slug: string) => {
    ...
    productStore.searchCategory(selectedCategory.value)
}

</script>
<template>
    <input placeholder="Search" v-model="search" @input="handleSearch(search)" type="text"
        class="border border-[#9f9f9f] rounded-2xl py-2 px-8 w-1/3 outline-none focus:border-black my-6">
    <div class="flex gap-2 overflow-x-scroll">
        <button v-for="(category, index) in productStore.categories" @click="categoryChangeHandler(category.slug)"
            :key="index" class="border border-[#9f9f9f] rounded-4xl py-2 px-6 whitespace-nowrap cursor-pointer"
            :class="{ 'border-black': selectedCategory == category.slug }">
            {{ category.name }}
        </button>
    </div>
    <section class="grid grid-cols-4 gap-4">
        <p v-if="productStore.loading">Products loading...</p>
        <ProductCard v-for="product in productStore.currentProducts" :product="product" :key="product.id"></ProductCard>
    </section>
